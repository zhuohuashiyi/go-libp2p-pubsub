...
func NewGossipSub(ctx context.Context, h host.Host, peerDist map[peer.ID]float64, truncRadio float64, opts ...Option) (*PubSub, error) {
	rt := DefaultGossipSubRouter(h)
	rt.peerDist = peerDist
	rt.truncRadio = truncRadio
...
func (gs *GossipSubRouter) emitGossip(topic string, exclude map[peer.ID]struct{}) {
	...
	peers = truncPeers(peers, gs.peerDist, gs.truncRadio, target)
	// target类似于gossipNodes,这里是为了避免peers本身数量不够target
	if target > len(peers) {
		target = len(peers)
	} else {
		//shufflePeers 类似于洗牌，打乱peers的顺序
		shufflePeers(peers)
	}
	// 取最终需要发送gossip消息的peers
	peers = peers[:target]
...
func truncPeers(peers []peer.ID, peerDist map[peer.ID]float64, truncRadio float64, target int) []peer.ID {
	// 先按照延时从小到大排序peers
	sort.Slice(peers, func(i, j int) bool {
		var t1, t2 float64
		var isPresent bool
		// 排序过程中需要注意节点不在peerDist(来自上层的传入)
		// 节点不在应当视为延时无限大，而不是零
		if t1, isPresent = peerDist[peers[i]]; isPresent {
			return false
		}
		if t2, isPresent = peerDist[peers[j]]; isPresent {
			return true
		}
		// 这里适当地考虑了排序稳定的问题
		if t1 != t2 {
	...
	// 计算截取的实际长度L,相当于将通信延时最小的L个作为直接邻居节点列表
	truncLen := min(max(int(float64(len(peers)) * truncRadio), target), len(peers))
	// 按照truncRadio截取peers,注意此处的truncRadio可能和最终的性能有关系，应当视为超参数
	peers = peers[: truncLen]
	return peers
}
...
func (gs *GossipSubRouter) getPeers(topic string, count int, filter func(peer.ID) bool) []peer.ID {
	...
	peers = truncPeers(peers, gs.peerDist, gs.truncRadio, count)
	shufflePeers(peers)
